...
Pour les élèves du campus
=========================

Polr est un "raccourcisseur d'URL". 

Install sur poste de développeur 
--------------------------------
...
Créer une base de données MySQL (ex: `polr`) et la configurer la base de données dans le fichier .env
...
finaliser l'installation en éxécutant les migrations puis lancer le server 
...
```bash
# copier env.dev.sample et éditer si besoin les valeurs du .env
cp env.dev.sample .env
# créer un docker-compose.override.yml et éditer les valeurs si besoin
cp docker-compose.override.sample.yml docker-compose.override.yml
# lancer docker-compose
docker-compose up -d
# entrer dans le container "web"
make bash
# lancer l'installation (composer install, php artisan migrate, chmod,...)
make install
# créer un admin avec la requête sql suivante :
# update users set role='admin' where username='plv';
```
